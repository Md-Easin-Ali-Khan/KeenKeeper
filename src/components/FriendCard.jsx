...
const FriendCard = () => {
    return (
        <section className='max-w-6xl mx-auto mb-7 md:mb-20'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 justify-center gap-6'>
                {data.map((friend) => (
                    ...
                    <Link key={friend.id} href={`/${friend.id}`}>
                        <div className="bg-white text-center p-6 rounded-lg shadow-md flex flex-col items-center hover:-translate-y-1 hover:shadow-xl cursor-pointer transition duration-300">
                            <div className='mb-3'>
                            ...
                            <div className='space-y-2'>
                                ...
                                <p className={`badge text-white font-medium text-[12px]  py-1.5 px-2 rounded-full capitalize ${statusStyle[friend.status] || 'bg-gray-500'} `}>{friend.status}</p>
                            </div>
                        </div>
                    </Link>
                ))}
